...
(define (apply-k kont v)
  (cases cont kont
         ...
         [set!-k  (target env k)
                  (apply-k k (set-var-env env target v))]
         [def-k   (sym k)
                  (apply-k k (define-in-global sym v))]))
...
(define (eval-exp-cps exp env k)
  (cases expression exp
         ...
         [if1-exp     (condition arm0)      (apply-k k (eval-exp-cps condition env (if1-k arm0 env k)))]
         ...
         [set!-exp    (target val)  (eval-exp-cps val env (set!-k target env k))]
         [def-exp     (sym body)    (eval-exp-cps body env (def-k sym k))];(define-in-global sym (eval-exp-cps body env k))]
...
(define *prim-proc-names* '(+ - * / ++ add1 -- sub1 cons = < <= > >= not map or zero? list null? assq eq? equal? apply atom? length
                              ...
                              cdaaar cdaadr cdadar cdaddr cddaar cddadr cdddar cddddr call/cc exit-list))
...
(define (apply-prim-proc-cps prim-proc args env k)
  (case  prim-proc
    [(call/cc)   (apply-proc-cps (car args) (list (k-proc k)) env k)]
